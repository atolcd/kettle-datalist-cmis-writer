...
public class CMISDLWStep extends BaseStep implements StepInterface {
  ...
  public boolean processRow(StepMetaInterface smi, StepDataInterface sdi)
    ...
    if (first) {
      ...
      connectCMIS(meta.getUrlField(), meta.getUserField(),
          meta.getPasswordField(), meta.getRepId());
    }
  ...
  /*
   * method for creating items and inserting them in an existing alfresco
   * share datalist instance
   */
  private void createItem(Object[] r) {
    ...
    for (int cp = 0; cp < r.length; cp++) {
      ...
      // logBasic("r[] class : " + r[cp].getClass().toString());
      if (r[cp] == null) {
        continue;
      }
      ...
      // method for handling long
      if (r[cp] instanceof Long) {
        BigInteger bi = new BigInteger(r[cp].toString());
        propertiesList.add(of.createPropertyIntegerData(
            objectOrder[cp], bi));
        continue;
      }

      // method for handling Double
      if (r[cp] instanceof Double) {
        BigDecimal db = new BigDecimal((String) r[cp].toString());
        propertiesList.add(of.createPropertyDecimalData(
            objectOrder[cp], db));
        continue;
      }

      // method for handling Float
      if (r[cp] instanceof Float) {
        BigDecimal db = new BigDecimal((String) r[cp].toString());
        propertiesList.add(of.createPropertyDecimalData(
            objectOrder[cp], db));
        continue;
      ...
      // method for handling Boolean
      if (r[cp] instanceof Boolean) {
        propertiesList.add(of.createPropertyBooleanData(
            objectOrder[cp], (Boolean) r[cp]));
        continue;
      }

      /*
       * method for handling Date although it should not happen while
       * official date format is GregorianCalendar
       */
    ...
    try {
      data.session
          .getBinding()
          .getObjectService()
          .createDocument(repId, properties, meta.getListId(), null,
